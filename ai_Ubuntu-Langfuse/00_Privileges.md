...
sudo -l
getent group sudo
grep "^adminuser_langfuse:" /etc/passwd
```
- Group Memberships
    - adminuser_langfuse – Primary user group.
    ...
    - lxd – Can manage LXD contadminuser_langfuseners. Membership in this group is effectively privileged because it can be used to gadminuser_langfusen root access if LXD is configured.
## Rename the user
0. create a temp sudo user than
1. login to temp user and rename the adminuser_langfuse to adminuser
...
```sh
ssh tempadmin@192.168.3.60
password:
whoami
```
- Step 2: Rename adminuser_langfuse to adminuser and update password
```sh
sudo usermod -l adminuser adminuser_langfuse # login name
sudo groupmod -n adminuser adminuser_langfuse # primary group 
